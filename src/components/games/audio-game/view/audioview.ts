...
import { AudioGameLaunchMode, IAudioGameState } from "../types/audiocall-types";
...
export default class AudioView {
    ...
    // private timer: Timer;
    constructor() {
        this.wrapper = new Control(null, 'div', 'audio-game');
        this.content = new Control(this.wrapper.node, 'div', 'audio-game__content content');
    ...
    toggleSound() {
        (document.querySelector('.audio-game__sound') as HTMLElement).classList.toggle('audio-game__sound_active');
    ...
    //метод вызыв во входе игры
    drawWrapper(container: HTMLElement) {
        // container.innerHTML = '';
        const closeWrapper = new Control(null, 'div', 'audio-game__abort');
        const actions = new Control(closeWrapper.node, 'div', 'audio-game__actions');
        const closeBtn = new Control(closeWrapper.node, 'div', 'audio-game__close');
        const soundControl = new Control(actions.node, 'div', 'audio-game__sound');
        const fullScreen = new Control(actions.node, 'div', 'audio-game__fullscreen');
    ...
    drawWelcomePage(mode: AudioGameLaunchMode) {
        ...
        gameDescr.node.innerHTML = `
            «Аудиовызов» - это тренировка для восприятие на слух новых и уже изученных слов из вашего словаря.
        `;
